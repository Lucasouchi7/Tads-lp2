...
public class LivroService {
    ...
    public Livro cadastrar(Livro livro) {
        ...
        if (livro.getAnoPublicacao() <= 0) {
            throw new RegraDeNegocioException(
                    "O ano de publicação deve ser maior que 0."
            );
        }

        Optional<Livro> livroJaCadastradoOptional =
                this.livroRepository.buscarPorIsbn(livro.getIsbn());
    ...
    public Livro adicionarExemplar(Long livroId, String codigoDoExemplar) {
        ...
        Optional<Livro> livroOptional =
                this.livroRepository.buscarPorId(livroId);

        if (livroOptional.isPresent()) {
            Livro livro = livroOptional.get();

            livro.adicionarExemplar(
                    new Exemplar(codigoDoExemplar, livro)
            );
    ...
    public Livro adicionarAutor(Long livroId, String nomeAutor) {
        ...
        livroEncontrado.adicionarAutor(
                new Autor(nomeAutor)
        );
